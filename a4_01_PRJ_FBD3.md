...
``` 
##### [9]Vue前端如何在视图中展示另一个页面内容(component)  
##### [10]要求当内容小于一定高度时，容器设为最小固定高度
div设置为根据内容大小自适应，要求当内容小于一定高度时，容器设为最小固定高度；当内容高度大于最小容器高度小于最大容器高度时，容器高度根据内容高度自适应；当内容大于一定高度时，容器设为最大固定高度，并且显示遮挡层，遮盖未显示的内容部分。  
##### [11]评论回复数据库表不关联、不添加外键
vue+springboot前后端分离项目实现评论回复功能，需要建哪些数据库表，如果在前后端可以直接通过页面事件关联评论回复对应的用户，所必要的数据库表，能否在不关联其他表的内容的前提下创建

#### Vue+SpringBoot前后端分离项目 （五）功能小结  
#### 20250416 (1)页面  
1. 实现内容  
...
#### 20250424 (1)公告  
#### 20250424 (1)图表echarts  
#### 20250424 (1)批量删除  
#### 20250424 (1)模块关联  
#### 20250424 (1)预约  
#### 20250424 (1)审核  
#### 20250424 (1)编辑wangeditor  
#### 20250424 (1)协同文档  
#### 20250424 (1)导出excel  

#### Vue+SpringBoot前后端分离项目 （六）代码小结  
#### 20250315 (1)查(查询全部)  
